...
class Home extends React.Component {
  ...
  handleRetrievePet() {
    API.get('Customers', '/items/customers').then(apiResponse => {
      return Promise.all(apiResponse.map(async (customer) => {
        ...
        const [, , , key] = /(([^\/]+\/){2})?(.+)$/.exec(customer.picKey);

        const picUrl = customer.picKey && await Storage.get(key, { level: 'private' });

        return { ...customer, picUrl };
      }));
    }).then(apiResponse => {
  ...
  renderPet(customer, index) {
    const uri = customer.picUrl;

    return (
      <TouchableHighlight
        onPress={() => {
          this.props.navigation.navigate('ViewPet', { customer })
        }}
      ...
      >
        <View style={styles.petInfoContainer}>
          ...
          />
          <Text style={styles.petInfoName}>{customer.name}</Text>
        </View>
      </TouchableHighlight>
    )
  }
            ...
            <View style={styles.container}>
              <Text style={styles.title}>My Customers</Text>
              {
                typeof apiResponse === 'string' ?
                  ...
                  apiResponse.map((customer, index) => this.renderPet(customer, index))
              }
            </View>
          }
        </ScrollView>
