...
const CUSTOMERS_TABLE_NAME = `${process.env.MOBILE_HUB_DYNAMIC_PREFIX}-customers`;
...
app.get('/items/customers', (req, res) => {
...
app.post('/items/customers', (req, res) => {
  if (!req.body.name) {
    res.status(400).json({
      message: 'You must specify a customer name',
    }).end();
    return;
  }
  ...
  Object.keys(customer).forEach(key => (customer[key] === '' && delete customer[key]));

  customer.userId = req.apiGateway.event.requestContext.identity.cognitoIdentityId || UNAUTH;
  ...
  dynamoDb.put({
    ...
    if (err) {
      console.log(err)
      res.status(500).json({
        message: 'Could not insert customer'
      }).end();
    } else {
      res.json(customer);
    }
